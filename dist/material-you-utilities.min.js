...
`,""]);let s=o},616:function(e,t,r){r.d(t,{Z:()=>s});var a=r(468),i=r.n(a),n=r(360),o=r.n(n)()(i());o.push([e.id,`/* Hide navigation bar */
html {
	--navbar-display: none;
}
`,""]);let s=o},334:function(e,t,r){r.d(t,{Z:()=>s});var a=r(468),i=r.n(a),n=r(360),o=r.n(n)()(i());o.push([e.id,`:host {
	--footer-height: 80px;
}
...
`,""]);let s=o},46:function(e,t,r){r.d(t,{Z:()=>s});var a=r(468),i=r.n(a),n=r(360),o=r.n(n)()(i());o.push([e.id,`:host {
...
Use a query string like ?i=1 (between 0 and 3 inclusive) to choose alternate color options.`,init:{config:{icon:"mdi:image",min:0,max:255}},card:{config:{text:{}},tabBarIndex:2},handler:tM},css_file:{domain:"input_text",default:"",name:"CSS Path/URL",description:"Path/URL to a CSS file to apply to the page and iframe roots for defining custom properties.",init:{config:{icon:"mdi:language-css3",min:0,max:255}},card:{config:{text:{}},tabBarIndex:2},handler:rY}};async function rJ(e,t,r){return await e.callWS({type:`${t}/create`,...r})}async function rQ(e,t,r,a){return await e.callWS({type:`${t}/update`,[`${t}_id`]:r,...a})}async function r0(e,t,r){await e.callWS({type:`${t}/delete`,[`${t}_id`]:r})}async function r1(e){let t=await e.sendMessagePromise({type:"config/label_registry/list"});return t.sort((e,t)=>e.name.localeCompare(t.name)),t}async function r2(e,t){await e.callWS({type:"config/label_registry/create",...t})}async function r4(e,t,r){await e.callWS({type:"config/entity_registry/update",entity_id:t,...r})}async function r5(e,t){if(!t)return!0;let r=new Event("hass-action",{bubbles:!0,composed:!0});return r.detail={action:"tap",config:{tap_action:{action:"fire-dom-event",confirmation:t}}},e.dispatchEvent(r),new Promise(e=>{let t,r=()=>{clearTimeout(t),window.removeEventListener("ll-custom",a),window.removeEventListener("dialog-closed",i)},a=()=>{r(),e(!0)};window.addEventListener("ll-custom",a);let i=()=>{t=setTimeout(()=>{r(),e(!1)},100)};window.addEventListener("dialog-closed",i)})}function r3(e,t="info"){return Z`<ha-alert .title="${e}" .alertType="${t}"></ha-alert>`}async function r6(e){return new Promise(async t=>{let r=(await tk()).hass,a=r.user?.id,i=window.browser_mod?.browserID?.replace(/-/g,"_");if(r.connection.connected&&a){let n=[];for(let e in rX){let t=n.find(t=>t.handler==rX[e].handler);t?t.inputs.push(e):(t={inputs:[e],handler:rX[e].handler},n.push(t))}let o=[];for(let t of n){let n=[];if(e.id?n=[...t.inputs.map(t=>tC(t,e.id))]:(n=[...t.inputs.map(e=>tC(e)),...t.inputs.map(e=>tC(e,a))],i&&n.push(...t.inputs.map(e=>tC(e,i)))),r.user?.is_admin)o.push(r.connection.subscribeMessage(()=>t.handler(e),{type:"subscribe_trigger",trigger:{platform:"state",entity_id:n}},{resubscribe:!0}));else for(let a of n)o.push(r.connection.subscribeMessage(r=>{"error"in r&&(console.error(r.error),t$(r.error)),t.handler(e)},{type:"render_template",template:`{{ states("${a}") }}`,entity_ids:a,report_errors:!0}))}t(Promise.all(o))}else setTimeout(()=>t(r6(e)),100)})}var r8=function(e,t,r,a){var i,n=arguments.length,o=n<3?t:null===a?a=Object.getOwnPropertyDescriptor(t,r):a;if("object"==typeof Reflect&&"function"==typeof Reflect.decorate)o=Reflect.decorate(e,t,r,a);else for(var s=e.length-1;s>=0;s--)(i=e[s])&&(o=(n<3?i(o):n>3?i(t,r,o):i(t,r))||o);return n>3&&o&&Object.defineProperty(t,r,o),o};customElements.get("disk-color-picker")||r(899);class r9 extends es{constructor(){super(...arguments),this.tabBarIndex=0,this.tabs=["theme","styles","other"],this.MODE_ID=`${tv}-mode`,this.unsubscribers=[]}async handleDeleteHelpers(e){if(!await r5(this,{text:"Are you sure you want to delete these helpers?"}))return;for(let e in rX){let t=tC(e,this.dataId);this.hass.states[t]&&await r0(this.hass,rX[e].domain,t.split(".")[1])}let t="Global input entities cleared";if(this.dataId){let e="";e=this.dataId==this.hass.user?.id?this.hass.user?.name??"":this.hass.states[this.dataId]?.attributes.friendly_name??this.dataId,t=`Input entities cleared for ${e}`}tE(this,t)}buildDeleteHelpersButton(){return Z`<div class="delete button" @click="${this.handleDeleteHelpers}">Delete Helpers</div>`}async handleCreateHelpers(e){let t="";for(let e in this.dataId&&(t=` ${this.hass.states[this.personEntityId??""]?.attributes?.friendly_name??this.personEntityId??this.dataId}`),rX){let r=tC(e,this.dataId);if(!this.hass.states[r]){let a=r.split(".")[1];await rJ(this.hass,rX[e].domain,{name:a,...rX[e].init.config}),await rQ(this.hass,rX[e].domain,a,{name:`${ty} ${rX[e].name}${t}`,...rX[e].init.config});let i=rX[e].domain,n=rG[i],o={entity_id:r};switch(i){case"input_text":case"input_number":o.value=rX[e].default;break;case"input_select":o.option=rX[e].default;break;case"input_boolean":n=`turn_${rX[e].default}`}await this.hass.callService(i,n,o),await r4(this.hass,r,{labels:[tb]})}}let r="Global input entities created";t&&(r=`Input entities created for${t}`),tE(this,r)}buildCreateHelpersButton(){return Z`<div class="create button" @click="${this.handleCreateHelpers}">Create Helpers</div>`}handleTabBar(e){let t=this.tabs.indexOf(e.detail.name);this.tabBarIndex!=t&&(this.tabBarIndex=t)}buildTabBar(e,t,r){return Z`<ha-tab-group @wa-tab-show="${t}">${r.map((t,r)=>Z`<ha-tab-group-tab slot="nav" panel="${t}" .active="${r==e}">${t}</ha-tab-group-tab>`)}</ha-tab-group>`}async handleSelectorChange(e){let t=e.target.getAttribute("field"),r=e.detail.value,a=rX[t].domain,i=rG[a],n={entity_id:tC(t,this.dataId)};switch(a){case"input_text":case"input_number":n.value=r||rX[t].default;break;case"input_select":n.option=r||rX[t].default}await this.hass.callService(a,i,n),this.requestUpdate()}buildSelector(e,t,r,a){let i,n=tC(t,this.dataId),o=this.hass.states[n]?.state;return i="input_boolean"===rX[t].domain?"on"==o:o??rX[t].default,Z`<ha-selector .hass="${this.hass}" .name="${e}" .selector="${r}" .value="${i??a}" .label="${e}" .placeholder="${a}" .required="${!1}" field="${t}" @value-changed="${this.handleSelectorChange}"></ha-selector>`}async handleKeyDown(e){!e.repeat&&["Enter"," "].includes(e.key)&&(e.preventDefault(),(e.target.parentElement?.className.replace("button","").trim()==="reset"?this.handleResetClick:this.handleMoreInfoClick).call(this,new window.MouseEvent("click",e),e.target))}async handleResetClick(e,t){let r=(e.target??t).getAttribute("field"),a=rX[r].domain,i=rG[a],n={entity_id:tC(r,this.dataId)};switch(a){case"input_text":case"input_number":n.value=rX[r].default;break;case"input_select":n.option=rX[r].default;break;case"input_boolean":i=`turn_${rX[r].default}`}await this.hass.callService(a,i,n),this.requestUpdate()}buildResetButton(e){return Z`<div class="reset button"><ha-icon @click="${this.handleResetClick}" @keydown="${this.handleKeyDown}" tabindex="0" field="${e}" .icon="${"mdi:restore"}"></ha-icon></div>`}handleMoreInfoClick(e,t){let r=tC((e.target||t).getAttribute("field"),this.dataId),a=new Event("hass-more-info",{bubbles:!0,cancelable:!0,composed:!0});a.detail={entityId:r},this.dispatchEvent(a)}buildMoreInfoButton(e){let t=tC(e,this.dataId),r=this.hass.states[t]?.attributes.icon;return Z`<pre class="description">${rX[e].description}</pre><div class="more-info button"><ha-icon @click="${this.handleMoreInfoClick}" @keydown="${this.handleKeyDown}" tabindex="0" field="${e}" .icon="${r}"></ha-icon></div>`}buildRow(e){let t=tC(e,this.dataId);if(!this.hass.states[t])return"";let r=this.hass.states[t]?.state;if("base_color"==e){let t,a=e=>{clearTimeout(t);let r=e.target,a=r.value;t=setTimeout(()=>{let e=new Event("value-changed");e.detail={value:a},r.dispatchEvent(e)},100)};return Z`<div class="column"><disk-color-picker field="${e}" value="${r}" @change="${a}" @keyup="${a}" @value-changed="${this.handleSelectorChange}"></disk-color-picker><div class="subrow"><div class="row">${this.buildMoreInfoButton(e)}<div class="label">${rX[e].name}</div></div><div class="row"><div class="label secondary">${r||rX[e].default}</div>${this.buildResetButton(e)}</div></div></div>`}let a="";"harmonize"==e&&(a=Z`<div class="row semantic-colors">${["pink","red","deep-orange","orange","amber","yellow","lime","light-green","green","teal","cyan","light-blue","blue","indigo","deep-purple","purple"].map(e=>Z`<div class="semantic-color" style="background:var(--${e}-color)"></div>`)}</div>`);let i=rX[e].card.config;return"input_number"==rX[e].domain?(i.min=this.hass.states[t]?.attributes?.min??-1,i.max=this.hass.states[t]?.attributes?.max??1,i.step=this.hass.states[t]?.attributes?.step??.01):"input_boolean"==rX[e].domain&&(r="on"==r),Z`${this.buildMoreInfoButton(e)} ${this.buildSelector(rX[e].name,e,i,r)}${a} ${rX[e].card.resetButton?this.buildResetButton(e):""}`}setupIds(){this.dataId&&!this.personEntityId&&(this.personEntityId=Object.keys(this.hass.states).find(e=>e.startsWith("person.")&&this.hass.states[e].attributes.user_id==this.dataId))}applyThemeMode(){if(this.darkMode!=this.hass.themes?.darkMode){this.darkMode=this.hass.themes?.darkMode;let e=this.hass.themes?.themes[ty].modes?.[this.darkMode?"dark":"light"]??{},t={};for(let[r,a]of Object.entries(e))t[`--${r}`]=a;rk(this,this.MODE_ID,rw(t))}}render(){this.setupIds();let e="Global";this.dataId&&(e=this.hass.states[this.personEntityId??""]?.attributes?.friendly_name??this.personEntityId??this.dataId);let t=Object.keys(rX).filter(e=>rX[e].card.tabBarIndex==this.tabBarIndex);this.hass.states[tC("spec",this.dataId)]?.state!="2025"&&(t=t.filter(e=>"platform"!=e));let r={};for(let e of t)r[e]=this.buildRow(e);for(let e in r)r[e]||delete r[e];return Z`<ha-card .hass="${this.hass}" .header="${e}">${this.personEntityId?Z`<div class="subtitle">ID: ${this.dataId}</div>`:""} ${this.buildTabBar(this.tabBarIndex,this.handleTabBar,this.tabs)}<div class="card-content">${Object.keys(r).length!=t.length?r3(this.hass.user?.is_admin?"Press Create Helpers to create and initialize inputs.":"Some or all input helpers not setup! Ask an Home Assistant administrator to do so.",this.hass.user?.is_admin?"info":"error"):""} ${Object.keys(r).map(e=>Z`<div class="row ${e}">${r[e]}</div>`)}</div>${this.hass.user?.is_admin?Z`<div class="card-actions">${this.buildCreateHelpersButton()}${this.buildDeleteHelpersButton()}</div>`:""}</ha-card><style id="${this.MODE_ID}"></style>`}firstUpdated(){let e=async()=>{let t=structuredClone(this.hass.themes?.themes[ty]);delete t.modes;let r=Object.entries(t);if(r.length){for(let[e,t]of r)this.style.setProperty(`--${e}`,t);return}setTimeout(()=>e(),100)};e();let t={targets:[this],id:this.dataId??""};for(let e of[tM,rV,rj,rY])e(t)}updated(){this.applyThemeMode();let e=`${tv}-theme`,t=this.shadowRoot?.querySelector("disk-color-picker");t&&!t.shadowRoot?.getElementById(e)&&rk(t,e,`
			...
			`)}connectedCallback(){super.connectedCallback(),r6({targets:[this],id:this.dataId??""}).then(e=>{this.unsubscribers=e})}disconnectedCallback(){super.disconnectedCallback(),this.unsubscribers.forEach(async e=>{e()})}static get styles(){return l`:host{width:var(--width);font-family:var(--font-family)}.card-content{display:flex;flex-direction:column;padding:16px;margin:0;gap:20px}.subtitle{margin-top:-24px;padding:0 16px 16px;color:var(--secondary-text-color);font-size:var(--md-sys-typescale-label-large-size,14px);font-weight:var(--md-sys-typescale-label-large-weight,500);line-height:var(
					...
					var(--mdc-fab-box-shadow)
						...
						var(--mdc-fab-box-shadow)
					)}.theme-mode-fab:hover::after{opacity:.08}}.theme-mode-fab:active::after,.theme-mode-fab:focus-visible::after{opacity:.18}`}}r7([eh()],ae.prototype,"hass",void 0),r7([eh()],ae.prototype,"narrow",void 0),r7([eh()],ae.prototype,"route",void 0),r7([eh()],ae.prototype,"panel",void 0),r7([ed()],ae.prototype,"tabBarIndex",void 0),r7([ed()],ae.prototype,"darkModeIndex",void 0),async function(){if(window.MaterialYouInit)return;window.MaterialYouInit=!0,r_(window),tT(document.querySelector("html"),`${ty} Utilities v${t.i8}`);let e=await tk();new MutationObserver(async t=>{for(let r of t)for(let t of r.addedNodes)if("IFRAME"==t.nodeName){let t=await tw(e.shadowRoot,"iframe"),r=await tx(t,"contentWindow");r_(r);let a=await tx(r,"document"),i={targets:[await tw(a,"body")]};for(let e of[rV,rj,rY])await e(i)}}).observe(e.shadowRoot,{subtree:!0,childList:!0}),customElements.define(`${tv}-config-card`,r9),customElements.define(`${tv}-panel`,ae);let r=async(e=10)=>{if(e>2e4)return;let t=(await tk()).hass,a=t?.themes?.theme;if(!a)return void setTimeout(()=>r(2*e),e);if(a.includes(ty)){let e={targets:[await tw(document,"html")]};for(let t of[tM,rV,rj,rY,rA])await t(e)}};r(),window.addEventListener("visibilitychange",async()=>{if(!document.hidden)for(let e of[rV,rj,rY])await e({})}),r6({});let a=async(e=10)=>{if(e>2e4)return;let t=(await tk()).hass,r=t.user?.id;if(!t.connection.connected||!r)return void setTimeout(()=>a(2*e),e);t.connection.subscribeEvents(()=>rV({}),"themes_updated"),t.user?.is_admin&&t.connection.subscribeEvents(e=>{e?.data?.service=="set_theme"&&setTimeout(()=>rV({}),1e3)},"call_service")};a()}()})()})();